...
function onSpawnNote(id, d, t, s)
    if not s then
        setPropertyFromGroup('notes', id, 'scale.x', 5.5)
        setPropertyFromGroup('notes', id, 'scale.y', 5.5)
    elseif s and not stringEndsWith(getProperty('notes.[' .. id .. '].animation.curAnim.name'), 'end') then
        setPropertyFromGroup('notes', id, 'scale.x', 3)
        setPropertyFromGroup('notes', id, 'scale.y', 6)
    end
end
